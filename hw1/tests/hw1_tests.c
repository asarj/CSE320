...
// New test cases for validargs
Test(basecode_tests_suite, validargs_error_test2) {
    int argc = 3;
    char *argv[] = {"bin/transplant", "-d", "-p", NULL};
    int ret = validargs(argc, argv);
    int exp_ret = -1;
    cr_assert_eq(ret, exp_ret, "Invalid return for validargs.  Got: %d | Expected: %d",
         ret, exp_ret);
...
Test(basecode_tests_suite, validargs_error_test3) {
    int argc = 4;
    char *argv[] = {"bin/transplant", "-d", "-p", "-c", NULL};
    int ret = validargs(argc, argv);
    int exp_ret = -1;
    cr_assert_eq(ret, exp_ret, "Invalid return for validargs.  Got: %d | Expected: %d",
         ret, exp_ret);
...
Test(basecode_tests_suite, validargs_error_test4) {
    int argc = 3;
    char *argv[] = {"bin/transplant", "-c", "-d", NULL};
    int ret = validargs(argc, argv);
    int exp_ret = -1;
    cr_assert_eq(ret, exp_ret, "Invalid return for validargs.  Got: %d | Expected: %d",
         ret, exp_ret);
...
Test(basecode_tests_suite, validargs_help_test2) {
    int argc = 6;
    char *argv[] = {"bin/transplant", "-h", "-x", "-y", "BLAHBLAHBLAH", "-z", NULL};
    int ret = validargs(argc, argv);
    int exp_ret = 0;
    int opt = global_options;
    int flag = 0x1;
